...
export default function CombatsPage() {
  ...
  const fetchCombats = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE}/liste`);
      if (response.ok) {
  ...
  const openCombatDetails = (combat: Combat) => {
  ...
  const saveTour = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/addTour`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
                        ...
                        {Array.from(new Set((selectedCombat.deroulement || []).map(t => t.tour))).map((tourNumber) => {
                          ...
                          return (
                            <div key={tourNumber} className="bg-gray-700 p-4 rounded border-l-4 border-blue-500">
                              <h4 className="text-lg font-medium text-yellow-300 mb-3">Tour {tourNumber}</h4>
                              ...
                              <div className="space-y-3">
                                {tourItems.map((item, index) => (
                                  <div key={index} className={`p-3 rounded ${
                                    ...
                                    {item.arbitre ? (
                                      <div>
                                        <p className="text-sm mb-2 whitespace-pre-line">{item.verdict?.texte || ''}</p>
                                      ...
                                      <div>
                                        <p className="text-sm mb-2 whitespace-pre-line">{item.contre?.texte || ''}</p>
